...
#define URL_SCHEME_FTP                                 @"ftp"
#define URL_SCHEME_FTPS                                @"ftps"

NSString *const XDefaultWhitelistRejectionString = @"ERROR whitelist rejection: url='%@'";

@implementation XWhitelist

@synthesize whitelist;
@synthesize expandedWhitelist;
@synthesize allowAll;
...
- (BOOL)isIPv4Address:(NSString *)externalHost
{
    ...
    NSUInteger octetsCount = [octets count];
    ...
    for (NSUInteger i = 0; i < octetsCount; ++i)
    {
        NSString* octet = [octets objectAtIndex:i];

        if ([octet isEqualToString:@"*"])
        {
            // passes - check next octet
            continue;
        }
        else if ([numberFormatter numberFromString:octet] == nil)
        {
            // fails - not a number and not within our range, return
            return NO;
        }
    }
...
- (void)processWhitelist
{
    ...
    {
        XLogE(@"ERROR: XWhitelist was not initialized properly, all urls will be disallowed.");
        return;
    }
    ...
    // FIXME: com.hk can't be matched correctly
    NSString *tldMatch = @"(aero|asia|arpa|biz|cat|com|coop|edu|gov|info|int|jobs|mil|mobi|museum|name|net|org|pro|tel|travel|xxx|[a-z][a-z])";

    while (externalHost = [enumerator nextObject])
    {
        NSString *regex = [self extractHostFromUrlString:externalHost];
        BOOL isIP = [self isIPv4Address:regex];
        ...
        // starts with wildcard match - we make the first '.' optional (so '*.org.wikipdia.foo' will match 'org.wikipdia.foo')
        NSString *prefix = @"*.";
        ...
        if (!isIP && [regex hasSuffix:@".*"])
        {
            // replace * with tldMatch
            regex = [regex stringByReplacingCharactersInRange:NSMakeRange([regex length] - 1, 1) withString:tldMatch];
        }
...
{
    return [scheme isEqualToString:URL_SCHEME_HTTP] ||
    [scheme isEqualToString:URL_SCHEME_HTTPS] ||
    [scheme isEqualToString:URL_SCHEME_FTP] ||
    [scheme isEqualToString:URL_SCHEME_FTPS];
}
...
- (BOOL)isUrlAllowed:(NSURL *)url
{
    ...
    while (regex = [enumerator nextObject])
    {
        ...
        if ([regexTest evaluateWithObject:urlHost] == YES)
        {
    ...
    XLogE(@"%@", [XWhitelist errorStringForUrl:url]);
...
{
    return [NSString stringWithFormat:XDefaultWhitelistRejectionString, [url absoluteString]];
}
