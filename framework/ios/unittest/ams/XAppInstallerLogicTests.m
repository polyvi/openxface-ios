...
- (void)testCopyEmbeddedJsFileToAppThrows
{
    STAssertThrows([self->appInstaller copyEmbeddedJsFileToApp:nil], nil);
    STAssertThrows([self->appInstaller copyEmbeddedJsFileToApp:@""], nil);
}

- (void)testCopyEmbeddedJsFileToAppWhenAppIsNative
{
    XAppInfo *appInfo = [[XAppInfo alloc] init];
    [appInfo setAppId:XAPPINSTALLER_LOGIC_TESTS_NATIVE_APP_PACKAGE_APP_ID];
    [appInfo setType:APP_TYPE_NAPP];
    ...
    NSString *destDir = [XUtils buildWorkspaceAppSrcPath:XAPPINSTALLER_LOGIC_TESTS_NATIVE_APP_PACKAGE_APP_ID];
    NSString *appsDirPath = [[XConfiguration getInstance] appInstallationDir];
    NSString *destJsPath = [appsDirPath stringByAppendingFormat:@"%@%@%@", XAPPINSTALLER_LOGIC_TESTS_NATIVE_APP_PACKAGE_APP_ID, FILE_SEPARATOR, XFACE_JS_FILE_NAME];

    // 测试前检查
    NSFileManager *fileManager = [NSFileManager defaultManager];
    [fileManager createDirectoryAtPath:destDir withIntermediateDirectories:YES attributes:nil error:nil];
    STAssertFalse([fileManager fileExistsAtPath:destJsPath], nil);

    // 执行测试
    STAssertNoThrow([self->appInstaller copyEmbeddedJsFileToApp:XAPPINSTALLER_LOGIC_TESTS_NATIVE_APP_PACKAGE_APP_ID], nil);

    // 测试后检查
    STAssertTrue([fileManager fileExistsAtPath:destJsPath], nil);

    // 清理环境
    [XFileUtils removeItemAtPath:destDir error:nil];
}

- (void)testCopyEmbeddedJsFileToAppWhenAppIsWeb
{
    XAppInfo *appInfo = [[XAppInfo alloc] init];
    [appInfo setAppId:XAPPINSTALLER_LOGIC_TESTS_APP_PACKAGE_APP_ID];
    [appInfo setType:APP_TYPE_XAPP];
    id<XApplication> app = [XApplicationFactory create:appInfo];
    [self->appList add:app];

    NSString *destDir = [XUtils buildWorkspaceAppSrcPath:XAPPINSTALLER_LOGIC_TESTS_APP_PACKAGE_APP_ID];
    NSString *appsDirPath = [[XConfiguration getInstance] appInstallationDir];
    NSString *destJsPath = [appsDirPath stringByAppendingFormat:@"%@%@%@", XAPPINSTALLER_LOGIC_TESTS_APP_PACKAGE_APP_ID, FILE_SEPARATOR, XFACE_JS_FILE_NAME];
    ...
    // 执行测试
    STAssertNoThrow([self->appInstaller copyEmbeddedJsFileToApp:XAPPINSTALLER_LOGIC_TESTS_APP_PACKAGE_APP_ID], nil);
